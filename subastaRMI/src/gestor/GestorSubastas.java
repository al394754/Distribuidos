...
public class GestorSubastas {
	...
	public Vector<String> consultaTipo(String tipo) throws IOException {	
		...
		for(int contadorArticulos = 0; contadorArticulos < cantidadTotal - 1; contadorArticulos ++) {
			Articulo articulo = new Articulo();
			articulo.leeDeFichero(stream);
			if(tipo.equals(articulo.rightPad(articulo.getCodart(), 2)) && !articulo.getVendido()){						//Comprobamos si las dos primeras letras de cada artículo coinciden con el tipo buscado
	...
	public String puja(String codcli, String codart, int puja) throws IOException {
		...
		Articulo articulo = new Articulo();
		articulo.leeDeFichero(stream);
		if(codcli.equals(articulo.rightPad(articulo.getCodven(), codcli.length())))//Comprobamos si el cliente y el codven son el mismo. Codcli tiene la longitud que se le pasa por teclado(máx. 8) pero codven siempre será de longitud 8, por lo que acortamos codven a la longitud del código que pasamos por teclado
			return respuesta;
		if(articulo.getVendido())
			return respuesta;
	...
	public String vende(String vendedor, String codart) throws EOFException, IOException {
		...
		if(articulo.rightPad(articulo.getCodcli(), 1).equals(" ")) 	//Con esta línea comprobamos que nadie ha pujado aún, si no hay nadie se almacena una cadena vacía(espacio)
			return respuesta;
		if(vendedor.equals(articulo.rightPad(articulo.getCodven(), vendedor.length()))) {  	//Comprobamos que el cliente que desea vender es el propietario		
